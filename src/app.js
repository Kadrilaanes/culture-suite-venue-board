(function(){
  ...
  var ui = {view:"territory", q:"", seg:"", reg:"", stage:"", sort:"stage", open:{}, terr:{}, map:"europe", sel:null};
  var api = null, saveTimer = null, readOnly = false, filesOK = true;

  /* ---------- theme ---------- */
  function applyTheme(){
    var saved = null;
    try { saved = localStorage.getItem("cs-venue-board-theme"); } catch(e){}
    if (saved === "light" || saved === "dark"){
      document.documentElement.dataset.theme = saved;
    } else {
      delete document.documentElement.dataset.theme; // follow system preference
    }
  }
  function toggleTheme(){
    var cur = document.documentElement.dataset.theme || (window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light");
    var next = cur === "dark" ? "light" : "dark";
    document.documentElement.dataset.theme = next;
    try { localStorage.setItem("cs-venue-board-theme", next); } catch(e){}
    // The MapLibre colours are resolved from CSS vars at build time — rebuild
    // so the map follows the theme change.
    if (ui.view === "territory"){ disposeMap(); render(); }
  }

  var $ = function(s){ return document.querySelector(s); };
  var cssVar = function(n){ return getComputedStyle(document.documentElement).getPropertyValue(n).trim() || "#1D1822"; };
  var esc = function(s){
      ...
      return getJSON("data/board.json").then(function(saved){
        ...
        try{
          ...
          if (raw){
            ...
            // Only accept schema-compatible saved boards; stale/map-less
            // state from older builds is discarded so the seed wins.
            if (j && j.schema >= 2 && Array.isArray(j.venues)){ if(!j.signals) j.signals=[]; state = j; }
            else { console.warn("cs-venue-board: discarding incompatible saved state"); }
          }
        }catch(e){}
      });
    }).then(function(){
  ...
  function mapHTML(){
    if (!MAPS) return "";
    // Interactive MapLibre map when the CDN libs are present; the static SVG
    // map stays as a zero-dependency fallback (offline / CDN blocked).
    if (!svgForced && typeof maplibregl !== "undefined" && typeof topojson !== "undefined") {
      return '<div class="maphead">'+mapSwitcherHTML()+
        '<div class="maplegend">'+
        ...
        '</div></div>'+
        '<div class="mapbox" id="imapbox"><div id="interactive-map"></div>'+
        '<div id="map-tip"></div></div>'+
        '<p class="map-status" id="map-status">Loading map…</p>'+
        '<div id="mapcallout">'+mapCalloutHTML()+'</div>'+
        (offmapCount() ? '<p class="offmap">'+offmapCount()+' venue'+(offmapCount()===1?"":"s")+
          ' not on the map yet — add a latitude and longitude on the venue to place '+(offmapCount()===1?"it":"them")+'.</p>' : "");
    }
    return svgMapHTML();
  }

  function offmapCount(){
    return state.venues.filter(function(x){ return x.lat==null || x.lon==null; }).length;
  }
  function mapSwitcherHTML(){
    var keys = Object.keys(MAPS).filter(function(k){ return state.venues.some(function(x){ return mapOf(x)===k; }); });
    if (keys.length<2) return "";
    return '<div class="mapswitch">'+keys.map(function(k){
      var n = state.venues.filter(function(x){ return mapOf(x)===k; }).length;
      return '<button type="button" class="btn mini" data-map="'+k+'" aria-pressed="'+(ui.map===k)+'">'+
        esc(MAPMETA[k].n)+' <span class="n">'+n+'</span></button>';
    }).join("")+'</div>';
  }
  function mapCalloutHTML(){
  ...
  // Loaded once, reused across Territory re-renders. When the CDN geometry is
  // unreachable we fall back to the static SVG map rendered by svgMapHTML().
  var WORLD_TOPO_URL = "https://cdn.jsdelivr.net/npm/world-atlas@2/countries-50m.json";
  var REGION_BOUNDS = {
    europe: [[-24.5, 34.0], [27.0, 66.5]],   // Reykjavík → Vilnius
    na:     [[-165.0, 18.0], [-50.0, 55.0]]  // Hawaii → Newfoundland
  };
  var map = null, worldTopo = null, mapHoverId = null, mapReady = false, mapTipEl = null;
  var svgForced = false, mapBuilding = false;

  // Country code -> ISO 3166-1 numeric id used by world-atlas features.
  var ISO_NUM = { UK:826, IE:372, NL:528, BE:56, DE:276, DK:208, SE:752, NO:578,
    FI:246, IS:352, EE:233, LV:428, LT:440, US:840 };

  function initInteractiveMap(){
    var el = $("#interactive-map");
    if (!el) return;
    if (typeof maplibregl === "undefined") return;
    // A re-render of #view replaces the container. If the map's old container
    // is no longer attached, rebuild; otherwise just refresh the data.
    if (map){
      if (map.getContainer().isConnected){ refreshMapData(); return; }
      disposeMap();
    }
    // Guard against a second async build while one is already in flight.
    ...
    if (!worldTopo){
      mapBuilding = true;
      fetch(WORLD_TOPO_URL).then(function(r){ return r.ok ? r.json() : Promise.reject(new Error("topo")); })
        .then(function(t){ worldTopo = t; mapBuilding = false; buildMap(); })
        .catch(function(){
          mapBuilding = false;
          if (status) status.textContent = "Geometry offline — SVG map shown below.";
          svgForced = true;
          if (ui.view === "territory") render();
        });
    } else {
      buildMap();
    }
    ...
    function buildMap(){
      var statusEl = $("#map-status");
      try {
        var world = topojson.feature(worldTopo, worldTopo.objects.countries);
        world.features.forEach(function(f){ f.id = Number(f.id) || 0; });

        // MapLibre paint expressions need concrete colours — resolve the
        // board's CSS variables once so the map follows both themes.
        var C = {
          off: cssVar("--map-off"), line: cssVar("--map-line"),
          on: cssVar("--map-on"), accent: cssVar("--accent"),
        ...
        map = new maplibregl.Map({
          ...
          style: { version: 8, sources: {}, layers: [
            { id: "bg", type: "background", paint: { "background-color": C.off } }
          ]},
          center: [10, 50], zoom: 3.2, attributionControl: false,
        ...
        map.on("load", function(){
          ...
          map.addLayer({ id: "world-fill", type: "fill", source: "world",
            paint: { "fill-color": C.off, "fill-opacity": 0.95 } });
          map.addLayer({ id: "world-line", type: "line", source: "world",
            paint: { "line-color": C.line, "line-width": 0.5 } });

          // Venue-count choropleth (country tint = at least one venue)
          map.addSource("venue-country", { type: "geojson", data: { type: "FeatureCollection", features: [] } });
          map.addLayer({ id: "vc-fill", type: "fill", source: "venue-country",
            paint: {
              "fill-color": ["get", "fill"],
              "fill-opacity": ["case", ["boolean", ["feature-state", "hover"], false], 0.95, 0.75],
            }});
          map.addLayer({ id: "vc-line", type: "line", source: "venue-country",
            paint: { "line-color": C.line, "line-width": 0.7 } });

          // Venue dots (client = accent, prospect = outline)
          map.addSource("pins", { type: "geojson", data: { type: "FeatureCollection", features: [] } });
          map.addLayer({ id: "pins-glow", type: "circle", source: "pins",
            paint: {
              "circle-radius": ["+", 6, ["*", 2.1, ["sqrt", ["get", "r"]]]],
              "circle-color": ["case", ["==", ["get", "client"], 1], C.accent, C.muted],
              "circle-opacity": 0.15, "circle-blur": 0.7,
            }});
          map.addLayer({ id: "pins-core", type: "circle", source: "pins",
            paint: {
              "circle-radius": ["+", 2, ["*", 0.95, ["sqrt", ["get", "r"]]]],
              "circle-color": ["case", ["==", ["get", "client"], 1], C.accent, C.surface],
              "circle-stroke-color": ["case", ["==", ["get", "client"], 1], C.accent, C.muted],
              "circle-stroke-width": 1.6,
            }});
          map.addLayer({ id: "pins-label", type: "symbol", source: "pins", minzoom: 6,
            layout: {
              "text-field": ["get", "name"], "text-size": 10.5,
              "text-offset": [0, 1.4], "text-anchor": "top",
              "text-allow-overlap": false, "text-optional": true,
            },
            paint: { "text-color": C.ink2, "text-halo-color": C.surface, "text-halo-width": 1.4 } });

          wireMapEvents();
          refreshMapData();
          fitRegion(ui.map || "europe");
          if (statusEl) statusEl.textContent = "";
          mapReady = true;
        });
      } catch (e) {
        if (statusEl) statusEl.textContent = "Map failed: " + (e && e.message ? e.message : e);
        console.error("interactive map error", e);
      }
    }
  }
  ...
  function refreshMapData(){
    ...
    // Choropleth: country fill = map-on when it holds ≥1 venue
    var onFill = cssVar("--map-on");
    var byCountry = {};
    state.venues.forEach(function(v){ if (v.lat!=null && v.lon!=null) byCountry[v.country] = (byCountry[v.country]||0)+1; });
    var world = topojson.feature(worldTopo, worldTopo.objects.countries);
    var vc = world.features.filter(function(f){
      var iso = ISO_NUM[f.id];
      return iso && byCountry[iso];
    }).map(function(f){
      return {
        type: "Feature",
        properties: { fill: onFill, count: byCountry[ISO_NUM[f.id]] },
        geometry: f.geometry,
      };
    });
    map.getSource("venue-country").setData({ type: "FeatureCollection", features: vc });

    // Pins
    var pins = state.venues.filter(function(v){ return v.lat!=null && v.lon!=null; }).map(function(v){
      return {
        type: "Feature",
        properties: {
          id: v.id, name: v.name, city: v.city, country: v.country,
          client: v.segment==="client" ? 1 : 0,
          r: v.tier==="flagship" ? 7 : v.tier==="mid" ? 5.5 : 4.5,
          stage: STAGEMAP[v.stage] ? STAGEMAP[v.stage].n : v.stage,
          stagec: STAGEMAP[v.stage] ? STAGEMAP[v.stage].c : "#8E8397",
          value: money(v.value),
        },
  ...
  function fitRegion(k){
    if (!map || !REGION_BOUNDS[k]) return;
    map.fitBounds(REGION_BOUNDS[k], { padding: 24, duration: 700 });
  }

  function wireMapEvents(){
    if (!map) return;
    map.on("mousemove", "vc-fill", function(e){
      map.getCanvas().style.cursor = "pointer";
      if (mapHoverId !== null) map.setFeatureState({ source: "venue-country", id: mapHoverId }, { hover: false });
      mapHoverId = e.features[0].id;
      map.setFeatureState({ source: "venue-country", id: mapHoverId }, { hover: true });
    });
    map.on("mouseleave", "vc-fill", function(){
      map.getCanvas().style.cursor = "";
      if (mapHoverId !== null){ map.setFeatureState({ source: "venue-country", id: mapHoverId }, { hover: false }); mapHoverId = null; }
    });

    map.on("mousemove", "pins-core", function(e){
      map.getCanvas().style.cursor = "pointer";
      var f = e.features[0];
      var st = { color: f.properties.stagec, name: f.properties.stage };
      if (mapTipEl){
        mapTipEl.innerHTML = "<b>"+esc(f.properties.name)+"</b>"+
          esc(f.properties.city+", "+f.properties.country)+
          " <span class=\"t\" style=\"background:"+st.color+"22;color:"+st.color+"\">"+esc(st.name)+"</span>"+
          " · "+esc(f.properties.value);
        mapTipEl.style.display = "block";
        var box = $("#imapbox").getBoundingClientRect();
        mapTipEl.style.left = Math.min(e.point.x+14, box.width-250) + "px";
        mapTipEl.style.top = (e.point.y+10) + "px";
      }
    });
    map.on("mouseleave", "pins-core", function(){
      map.getCanvas().style.cursor = "";
      if (mapTipEl) mapTipEl.style.display = "none";
    });

    map.on("click", "pins-core", function(e){
      var id = e.features[0].properties.id;
      ui.sel = (ui.sel===id) ? null : id;
      var co = $("#mapcallout");
      if (co) co.innerHTML = mapCalloutHTML();
    });
  }

  function disposeMap(){
    if (map){ try{ map.remove(); }catch(e){} map = null; mapReady = false; mapHoverId = null; }
  }
  ...
  function render(){
    ...
    if (ui.view === "territory") initInteractiveMap();
  }
  ...
  $("#tabs").addEventListener("click", function(e){
    ...
    if (ui.view !== "territory") disposeMap();
  ...
  $("#view").addEventListener("click", function(e){
    ...
    if (mb){
      ui.map=mb.dataset.map; ui.sel=null;
      if (map && map.getContainer().isConnected){
        // Interactive mode: just refit + refresh, keep the map instance.
        fitRegion(ui.map);
        render();
      } else {
        render();
